...
def stats():
   ...
   #friends=[]
    ...
    if request.method == 'POST' and "slider" in request.form:
        ...
        comment_dates = []
        dict_comment_timestamp_friend =[]
        listcommentnames =[]
        listcommenttime =[]
        with open(os.path.join(APP_ROOT, 'data/comments.json')) as file10:
            data_comments = json.load(file10)
            prev =''
            names1 = []
            print(usersplit)
            for i in data_comments.get('comments'):
                ...
                for a in res:
                    if (a not in usersplit and a != 'Ladias' and a != 'likes'
                    and a != 'liked' and a != 'commented' and a != 'his' and a != 'her' and a != 'own' and a != 'photo' 
                    and a != 'comment' and a != 'post' and a != 'video' and a != 'a' and a != 'replied'
                    and a != 'on' and a != 'timeline'and a != 'album' and a != 'in' and count <= 1
                    and a != 'reacted' and a != 'to'):
                        if count == 1:
                            names1.append(str(prev +' '+ a))
                            temp = datetime.date.fromtimestamp(i.get('timestamp'))
                            #dict_comment_timestamp_friend.update({str(prev +' '+ a): temp})
                            dict_comment_timestamp_friend.append([str(prev +' '+ a), temp])
                            listcommentnames.append(str(prev +' '+ a))
                            listcommenttime.append(temp)
                        prev = a
                        count = count+1
            listtimefriend=[listcommentnames,listcommenttime]
            """friendCountFromSlider = request.form['slider']
            print("slider  " + friendCountFromSlider)"""
            counter = Counter(listcommentnames)
            most_occur = counter.most_common(int(friendCountFromSlider))
            most_occured_names = []
            for i in most_occur:
                most_occured_names.append(i[0])
            ...
            listcommentnames_output = []
            listcommenttime_output = []
            print(listtimefriend)
            for elem, elem1 in zip(listcommentnames,listcommenttime):
                print(elem)
                if elem in most_occured_names:
                    listcommentnames_output.append(elem)
                    listcommenttime_output.append(elem1)
            
            minimuim = react_timestamps[0]
            counter = 0
            comment_count_according_to_date=[]

            for a in listcommenttime_output: 
            # first and last not being recorded
                if minimuim >= a + datetime.timedelta(days=+30):
                    comment_dates.append(a.strftime("%m/%Y"))
                    comment_count_according_to_date.append(counter)
 ...
 ####           
    ...
    line_chart1.x_labels = map(str, react_dates)
    ...
    if request.method == 'POST' and "postline" in request.form:
        print("line")
        graphpng = graph.render_to_png('flaskd3/static/graph_pics/line.png')# replace pie.png with variable filename: next line also
        postline.image_file = 'line.png'
    ...
    if request.method == 'POST' and "postbar" in request.form:
        ...
        #graph_file = save_picture('graph_pics', piepng)
        image_file = url_for('static', filename='graph_pics/' + postbar.image_file)
    ...
    if request.method == 'POST' and "postpie" in request.form:
        ...
        #graph_file = save_picture('graph_pics', piepng)
        image_file = url_for('static', filename='graph_pics/' + postpie.image_file)
    ...
    #sgraphpng = graph.render_to_png('flaskd3/static/graph_pics/graph.png')
    return render_template("stats.htm", graph_data=graph_data, line_data=line_data, line_data1=line_data1,
     pie=pie, Averag_reacts_per_months=Averag_reacts_per_months, Averag_comments_per_months=Averag_comments_per_months,
